...
**🎉 Готово!** Откройте браузер: **http://localhost/** (через Nginx) или **http://localhost:8000** (прямой доступ к frontend)
...
> ⚠️ **Важно:** ActivityWatch должен быть установлен и запущен на вашем компьютере.  
> Скачать: https://activitywatch.net/

**📖 Подробная документация:** [docs/QUICKSTART.md](docs/QUICKSTART.md)
...
- **Через Nginx (рекомендуется):** http://localhost/ - единая точка входа для всех сервисов
- **Прямой доступ к frontend:** http://localhost:8000 
...
- **Hot Module Replacement:** Поддерживается в dev режиме
...
```bash
make setup      # Настроить проект (создать .env файлы)
make start      # Запустить все сервисы (Frontend + Backend + Monitoring)
make stop       # Остановить все сервисы
...
```bash
make build-aw   # Собрать клиент для текущей ОС (macOS/Linux)
make run-aw     # Запустить сбор данных за последний час
```

⚠️ **Важно:** ActivityWatch должен быть установлен и запущен.  
📖 Подробная документация: [docs/ACTIVITYWATCH.md](./docs/ACTIVITYWATCH.md)
